...
func ProvideCmd() *Cmd {
	...
	workorderProcessService := service4.NewWorkorderProcessService(processDAO, workorderFormDesignDAO, workorderCategoryDAO, workorderInstanceDAO, logger)
	workorderProcessHandler := api6.NewWorkorderProcessHandler(workorderProcessService)
	templateDAO := dao3.NewTemplateDAO(db, logger)
	templateService := service4.NewTemplateService(templateDAO, userDAO, processDAO, workorderCategoryDAO, workorderInstanceDAO, logger)
	templateHandler := api6.NewTemplateHandler(templateService)
	serviceInstanceService := service4.NewInstanceService(workorderInstanceDAO, processDAO, logger)
	instanceHandler := api6.NewInstanceHandler(serviceInstanceService)
	instanceFlowDAO := dao3.NewInstanceFlowDAO(db, logger)
	instanceFlowService := service4.NewInstanceFlowService(instanceFlowDAO, logger)
	...
	notificationDAO := dao3.NewNotificationDAO(db, logger)
